...
export const Card: React.FC<CardProps> = ({ title, description, icon, className = "", children }) => {
  return (
    <div className={`p-6 bg-white dark:bg-dark-card border border-gray-100 dark:border-dark-border rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-300 h-full flex flex-col cursor-pointer hover:shadow-lg hover:shadow-brand-500/20 ${className}`}>
      {icon && (
